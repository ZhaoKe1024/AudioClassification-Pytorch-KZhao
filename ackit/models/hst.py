...
class HSTModel(nn.Module):
    def __init__(self, configs):
        ...
        self.patch_parts = None
        self.dropout = None
        self.layers = None
        self.norm_layer = None
        self.avgpool_layer = None
        self.head = None
        self.__init_modules()
    ...
    def __init_modules(self):
        ...
        self.patch_parts = PatchPartition(img_size=img_size,
                                          ...
                                          img_channel=self.configs.model_conf["img_channel"],
                                          d=d)

        self.dropout = nn.Dropout(self.configs.model_conf["dropout_rate"])

        # build HST
        patch_res = self.patch_parts.patch_res
        self.patch_res = patch_res
        num_attention_heads = self.configs.model_conf["num_attention_heads"]
        num_blocks = self.configs.model_conf["num_blocks"]
        win_size = self.configs.model_conf["win_size"]
        drop_path_rate = self.configs.model_conf["drop_path_rate"]
        block_depths = [2 * depth for depth in num_blocks]
        stoch_dd_rates = [x.item() for x in
                          torch.linspace(0, drop_path_rate, sum(block_depths))]  # stochastic depth decay rule
        S = len(num_blocks)
        self.layers = nn.Sequential(*[
            HST_Layer(dimension=int(d * 2 ** i),
                      in_res=(patch_res[0] // (2 ** i), patch_res[1] // (2 ** i)),
                      num_attention_heads=num_attention_heads[i],
                      num_block=num_blocks[i],
                      win_size=win_size,
                      mlp_ratio=self.configs.model_conf["mlp_ratio"],
                      use_bias=self.configs.model_conf["use_bias"],
                      dropout=self.configs.model_conf["dropout_rate"],
                      attn_dropout=self.configs.model_conf["attn_dropout_rate"],
                      drop_path=stoch_dd_rates[sum(block_depths[:i]):sum(block_depths[:i + 1])],
                      patch_merge=PatchMerge if (i < S - 1) else None,
                      use_checkpoint=False)
            for i in range(S)
        ])

        feat_map_dim = int(d * 2 ** (S - 1))
        self.norm_layer = nn.LayerNorm(feat_map_dim)
...
class LWMSA(nn.Module):
    ...
    def __init__(self, win_size, dimension, num_attention_heads, use_bias=True, attn_dropout=0.0, proj_dropout=0.0):
        ...
        self.inter_token_interaction_matrix = nn.Parameter(
            torch.zeros(token_table_size[0] * token_table_size[1], num_attention_heads))
        ...
        relative_coords = flattened_coords[:, :, None] - flattened_coords[:, None,
                                                         :]  # (2,M**2,1)-(2,1,M**2)=(2,M**2,M**2)
    ...
    def forward(self, x, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            attention = attention.view(int(W_B / batch), batch, self.num_attention_heads, num_patch,
                                       num_patch) + attn_mask.unsqueeze(1).unsqueeze(0)
